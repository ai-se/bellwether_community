        ...
        for s_project in selected_projects:
            try:
                ...
                for i in range(1):
                    ...
                    #clf.fit(X,y)
                    clf = FFT.FFT('F1','Buggy',5)
                    clf.train, clf.test = df, df
                    clf.build_trees()
                    ...
                    for d_project in destination_projects:
                        try:
                            ...
                            test_df = self.prepare_data(d_path)
                            _df_test_loc = test_df.LOC
                            ...
                            clf.test = test_df
                            results = clf.find_best_tree()
                            print(results)
                            F['f1'] = results[6]
                            F['precision'] = results[0]
                            F['recall'] = results[1]
                            #F['g-score'] = results[6]
                            #F['d2h'] = results[6]
                            #F['pci_20'] = results[6]
                            ...
                            if 'f1' not in score.keys():
                                ...
                                score[d_project]['f1'].append(F['f1'])
                                score[d_project]['precision'].append(F['precision'])
                                score[d_project]['recall'].append(F['recall'])
...
if __name__ == "__main__":
    ...
    data_store_path = 'data/1385/exp1/extra/'
    bell = bellwether(path,meta_path)
    cluster,cluster_tree = bell.build_BIRCH()
    with open('data/1385/exp1/1385_cluster_4.pkl', 'rb') as handle:
        _cluster_projects = pickle.load(handle)
    cluster_ids = [4] # need to include cluster 1
