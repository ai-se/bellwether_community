# -*- coding: utf-8 -*-
"""
Created on Fri Apr  6 11:58:31 2018
...
class DE(object):
  ...
  def __init__(self, params_distribution, goal, target_class,
               num_population=60, repeats=100, f=0.75, cr=0.3, life=30):
  ...
  def Tune(self):
    def isBetter(new, old):
      return new < old if self.goal == "PE" else new > old
...
class DE_Tune_ML(DE):
  def __init__(self, learner, params_distribution, goal, target_class,
               num_population=60, repeats=100, f=0.75, cr=0.3, life=20):
    self.learner = learner
  ...
  def best(self):
    # sortlst = []
    if self.goal == "PE":  # the less, the better.
