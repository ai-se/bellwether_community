...
class bellwether(object):
    ...
    def run_bellwether(self,projects,all_projects):
        threads = []
        results = {}
        _projects = projects
        ...
        for i in range(self.cores):
            ...
            t = ThreadWithReturnValue(target = self.bellwether, args = [split_projects[i],all_projects])
            threads.append(t)
        for th in threads:
            th.start()
        for th in threads:
    ...
    def run(self,selected_projects,all_projects,cluster_id,data_store_path):
        print(cluster_id)
        final_score = self.run_bellwether(selected_projects,all_projects)
...
if __name__ == "__main__":
    ...
    x = 660
    _data_store_path = 'data/1385/exp_new_val/0/' + str(x) + '/'
    attr_dict = pd.read_pickle(meta_path)
    attr_df = pd.DataFrame.from_dict(attr_dict,orient='index')
    attr_df_index = list(attr_df.index)
    i = 10
    selected_projects = list(attr_df_index)
    data_store_path = _data_store_path + 'fold_' + str(i) + '/'
    bell = bellwether(path,attr_df)
    bell.run(selected_projects[x:x+38],selected_projects,0,data_store_path)
